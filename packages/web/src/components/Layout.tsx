...
interface NavItem {
  ...
  roles?: string[];
  // Any CallGuard platform staff (support + superadmin): support inbox, read-only analytics.
  staffOnly?: boolean;
  // Superadmin only: provisioning, tenant settings, staff management.
...
const navItems: NavItem[] = [
  ...
  { path: '/admin/tenants', label: 'Tenants', icon: 'M3 21h18M5 21V7l8-4v18M19 21V11l-6-3M9 9v0M9 12v0M9 15v0', superadminOnly: true },
  { path: '/admin/staff', label: 'Staff', icon: 'M16 21v-2a4 4 0 00-4-4H6a4 4 0 00-4 4v2M9 7a4 4 0 100 8 4 4 0 000-8zM22 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75', superadminOnly: true },
];
    ...
    <div className="flex h-screen">
      ...
      <aside className="w-[220px] bg-white border-r border-sidebar-border flex flex-col fixed left-0 top-0 h-screen">
        ...
        <nav className="flex-1 min-h-0 overflow-y-auto px-3 py-0.5">
          {navItems
            .filter((item) => {
              if (item.superadminOnly) return !!user?.is_superadmin;
