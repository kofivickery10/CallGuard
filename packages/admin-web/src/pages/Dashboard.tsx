...
export default function Dashboard() {
  ...
  const totalCostMtd = (data.platform_claude_cost_mtd + data.platform_deepgram_cost_mtd).toFixed(2);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-card p-5 border border-border inline-block">
        <p className="text-xs text-text-muted font-semibold uppercase tracking-wider mb-1">
          Total platform cost MTD
        </p>
        <p className="text-2xl font-bold text-text-primary">£{totalCostMtd}</p>
        <p className="text-xs text-text-muted mt-1">Claude + Deepgram combined estimate</p>
      </div>
    </div>
  );
}
